...
# --- 1. CONFIGURAÇÕES DE TRADUÇÃO E PALAVRAS-CHAVE ---
...
# --- 2. CONFIGURAÇÕES DE TEMPO E ARQUIVOS ORIGINAIS ---
    ...
    for tag_a in links_encontrados:
        ...
        if href not in vistos:
            ...
            if trad and trad != texto:
                file_append.write(f'<span class="sub-tra">↳ {trad}</span>')
...
if parser[5] is not None:
    ...
    gravar_box_estrito("collapseExample5", items, "https://apa.org", "APA Monitor")

# 6. Google Notícias
if parser[6] is not None:
    items = []
    for x in parser[6].find_all("article"): items.extend(x.find_all("a", class_="VDXfz"))
    gravar_box_estrito("collapseExample6", items, "https://google.com", "Google Notícias")
...
if parser[7] is not None:
    ...
    gravar_box_estrito("collapseExample7", items, "https://sbponline.org.br", "SBP")

# 8. Neuroscience
if parser[8] is not None:
    items = []
    for x in parser[8].find_all("h3"): items.extend(x.find_all("a"))
    gravar_box_estrito("collapseExample8", items, "", "Neuroscience")

# 9. Positive Psychology
if parser[9] is not None:
    gravar_box_estrito("collapseExample9", parser[9].find_all("a"), "", "Positive Psychology")
...
if parser[26] is not None: gravar_box_estrito("collapseExample26", parser[26].find_all("a", class_=class_amm), base_amm, "AMM Psico")
if parser[27] is not None: gravar_box_estrito("collapseExample27", parser[27].find_all("a", class_=class_amm), base_amm, "AMM Rel")
if parser[28] is not None: gravar_box_estrito("collapseExample28", parser[28].find_all("a", class_=class_amm), base_amm, "AMM Saúde")

# 29. Big Think
if parser[29] is not None:
    items = []
    for x in parser[29].find_all("h1", class_="card-headline"): items.extend(x.find_all("a"))
    gravar_box_estrito("collapseExample29", items, "", "Big Think")
